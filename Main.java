...
import javax.swing.JFrame; // java.awt.Frame can also be used
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // ~60 FPS game loop
        // Using a separate thread so the game run smooth without freezing UI
        // however, the main thread is also fine for simple games/animations
        new Thread() {
            @Override
            public void run() {
...
class Window {
    ...
    // Child components that are used in this window
    Label info;
    ...
    Window(int width, int height, String title) {        
        createWindowFrame(width, height, title);
        createRightPanel();
        createBottomPanel();
    ...
    private void createWindowFrame(int width, int height, String title) {
        windowFrame = new JFrame();
        windowFrame.setSize(new Dimension(width, height));
    ...
    private void createRightPanel() {        
        ...
        info = new Label(formattedHTMLString);
        info.setSize(new Dimension(150, 100));
        info.setLocation(25, 50);
        info.setFont(new Font("SansSerif", Font.BOLD, 14));
        info.setVisible(true);
        
        tfTarget = new TextField(); 
        tfTarget.setSize(150, 75);
        tfTarget.setLocation(25, 200);
        ...
        btnShoot.setSize(150, 75);
        btnShoot.setLocation(25, 300);
        btnShoot.setBackground(Color.GRAY);
        ...
        // creates the panel and adds all the above child components
    ...
    private void createBottomPanel() {        
        ...
        tfResult.setSize(550, 75);
        tfResult.setLocation(25, 20);
        tfResult.setVisible(true);
        tfResult.setFont(new Font("SansSerif", Font.BOLD, 20));
        
        btnPlayAgain = new Button("Play Again");
        btnPlayAgain.setSize(150, 75);
        btnPlayAgain.setLocation(625, 20);
...
class Field {
    ...
    private final Point area; // size of the field
    // these three objects from awt package are used to render 2D graphics
    private final Canvas canvas;
    private Graphics2D gfx;
    private BufferStrategy bfs;
    // custom objects related to game play
    
    Goal goal;
    Ball ball;
    Keeper keeper;

    // if you want to play with multiple Fields, it good idea to
    // create a more flexible constructor
    
    Field() {
        area = new Point(600, 400); // relative to top left (0, 0)
        canvas = new Canvas();
    ...
    void render() {
        ...
        // this null checking is VERY VERY important because when
...
class Ball {
    ...
    // this method checks to see if the Ball has hit its target corner
...
final class Keeper {
    ...
    // all the move the keeper can perform
    // these are basically the tfTarget Vectors
    final int[][] moves = {
        {-1, 0}, // left
        {1,  0}, //right
        {0, -1}, //jump up
